...
namespace StudyBuddy
{
    public partial class ShareDialog : Form
    {
        ...
        {
            var cleanedContent = CleanContentForSharing(_content);
            var formattedContent = FormatShareContent(cleanedContent);
            
            // Create a clean title without timestamp
            var cleanTitle = _title.Contains(" - ") ? _title.Substring(0, _title.LastIndexOf(" - ")) : _title;
            
            txtContent.Text = $"# {cleanTitle}\n\n{formattedContent}";
            
            // Apply better formatting to the text box
            txtContent.Font = new Font("Segoe UI", 11);
            txtContent.BackColor = Color.FromArgb(248, 249, 250);
            txtContent.ForeColor = Color.FromArgb(33, 37, 41);
        }

        private string CleanContentForSharing(string content)
        {
            ...
            foreach (var line in lines)
            {
                ...
                // Clean up formatting artifacts while preserving structure
                var cleaned = trimmed;
                
                // Convert to markdown-style formatting for sharing
        ...
        private string FormatShareContent(string content)
        {
            if (string.IsNullOrEmpty(content))
                return content;
            ...
            foreach (var line in lines)
            {
                ...
                // Format for clean markdown sharing
                ...
                {
                    formattedLines.Add($"  - {trimmedLine.Substring(4).Trim()}");
                }
